...
FAKE_DEVICES = [
    ...
    {
        "id": 7, "hostname": "ESP32-IoTSensor", "mac": "24:6F:28:AB:CD:11",
...
@fake_admin.route("/admin/login", methods=["POST"])
def admin_login():
    """Capture login credentials and redirect to dashboard."""
    _log_access("/admin/login")
...
@fake_admin.route("/admin/api/config", methods=["GET"])
def api_config_get():
    """Return fake router configuration JSON."""
    _log_access("/admin/api/config")
    return jsonify({
        "status": "success",
...
@fake_admin.route("/admin/api/config", methods=["POST"])
def api_config_post():
    """Capture any config changes attempted."""
    _log_access("/admin/api/config [POST]")
...
def register_admin_panel(
    ...
    log.info("Fake admin panel registered at /admin")
