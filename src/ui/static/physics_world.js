...
(function () {
  ...
  function spawnDevice(mac, rssi, channel, forceAttacker, vendor, ip) {
    ...
    const meta = aiAgent.registerDevice(body, mac, { rssi, channel, vendor, ip });
    ...
    if (data.devices) {
      for (const dev of data.devices) {
        if (dev.status !== "destroyed") {
          spawnDevice(dev.mac, dev.rssi, dev.channel, undefined, dev.vendor, dev.ip);
        }
      }
    }
  });
  ...
  function addDeviceToList(meta) {
    ...
    const vendorStr = meta.vendor && meta.vendor !== "Unknown" ? meta.vendor : "Unknown Vendor";
    const ipStr = meta.ip || "Unknown IP";
    ...
    entry.innerHTML = `
      <div style="display: flex; flex-direction: column; width: 100%;">
        <div style="display: flex; align-items: center;">
          ...
          <span class="device-rssi" style="margin-left: auto;">${meta.rssi} dBm</span>
        </div>
        <div style="font-size: 0.7rem; color: var(--text-muted); margin-left: 18px; margin-top: 4px;">
          ${ipStr} &middot; ${vendorStr}
        </div>
      </div>
    `;
